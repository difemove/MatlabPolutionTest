%% Con ayuda de la función plot_google_map ( thankas to Zohar Bar-Yehuda) se realiza la gráfica de los datos 
% de polución obtenidos sobre el mapa de bogotá. 
...
function polution(lat,lon,measure)
    ...
    for i=1:length(lat)-1
        ...
        elseif measure(i)>=352 && measure(i)<357
            plot(lon(i:i+1),lat(i:i+1),'LineStyle','-','Color',[0.7 0.2 0],'LineWidth',5)
        elseif measure(i)>=357.5
            plot(lon(i:i+1),lat(i:i+1),'LineStyle','-','Color',[0.7 0 0],'LineWidth',5)
         end
        hold on;
    end
   %  plot_google_map
